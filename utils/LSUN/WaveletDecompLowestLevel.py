...
def countZeroesLowestLevel(coeffs):
    count = 0
    totalLength = 0
    cA = coeffs[0]
    cD = coeffs[-1]
    for coeff in cD:
        totalLength += 3*coeff[0].size
        count += np.count_nonzero(coeff[0])+np.count_nonzero(coeff[1])+np.count_nonzero(coeff[2])
    percentageZeroes = (totalLength-count)/totalLength
    return count, totalLength, percentageZeroes

def countZeroesInDir(dir, numFiles=10, wavelets=["haar"], lowestLevel=False):
    ...
    for wavelet in wavelets:
        for coeffs in allCoefs[wavelet]:
            percentageZeroes = None
            if lowestLevel:
                count, totalLength, percentageZeroes = countZeroesLowestLevel(coeffs)
            else:
...
if __name__ == "__main__":
    ...
    # wavelets = ["haar"]
    reals = countZeroesInDir("../../data/test/Real",n,wavelets,lowestLevel=True)
    ddpm = countZeroesInDir("../../data/test/DDPM",n,wavelets,lowestLevel=True)
    ldm = countZeroesInDir("../../data/test/LDM", n, wavelets,lowestLevel=True)
    adm = countZeroesInDir("../../data/test/ADM", n, wavelets,lowestLevel=True)
    nRows = 2
    nCols = 4
    ...
    for i in range(0, len(wavelets)):
        ...
        axs[i // nCols][i % nCols].hist(ldm[wavelets[i]], bins=20, alpha=0.2, label="LDM")
    ...
    fig.suptitle(f"Percentage of zeroes in wavelet coefficients of {n} LSUN bedroom images lowest level")
