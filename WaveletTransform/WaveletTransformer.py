...
def getWaveletCoefficients(signal,wavelet,mode="smooth"):
    ...
    wave_dec = pywt.wavedec(signal, wavelet, mode=mode)
    hsm = wave_dec[0]
    wave_dct = {}
...
def inverseDWT(coeffs,wavelet,mode="smooth"):
    hsm, wave_dict = coeffs
    wave_levels = []
    number_of_levels = int(math.log2((len(wave_dict) + len(hsm))/len(hsm)))
    ...
    return pywt.waverec(to_reconstruct, wavelet, mode=mode)
